...
====================Beautified Below================
       commonCookie: prefix + "cookie/common",
        ...
        utils = {
            extend: function() {
                ...
                for (i = 1; i < len; i++)
                    if (null !== (options = arguments[i]))
                        for (name in options) void 0 !== (copy = options[name]) && (target[name] = copy);
                return target
            },
            ...
            param: function(o) {
                var ary = [];
                for (var key in o) ary.push(key + "=" + o[key]);
                return ary.join("&")
            },
            url: function(u, o) {
        ...
        setCookie = function(name, value, domain) {
            var exp = new Date;
            exp.setTime(exp.getTime() + 2592e6), document.cookie = domain ? name + "=" + escape(value) + ";domain=" + domain + ";path=/;expires=" + exp.toGMTString() : name + "=" + escape(value) + ";domain=sohu.com;path=/;expires=" + exp.toGMTString()
        },
    ...
    return SohuPassport.prototype = {
        ...
        createIFrame: function(name) {
            ...
            var ifa;
            return browser.ie && browser.ie < 9 ? ifa = document.createElement('<iframe src="about:blank" name="' + name + '"></iframe>') : (ifa = document.createElement("iframe"), ifa.setAttribute("name", name)), ifa.style.display = "none", document.body.appendChild(ifa), ifa
        },
        createForm: function(formData) {
            var hidden, name, value, form = document.createElement("form");
            for (name in formData) value = formData[name], hidden = document.createElement("input"), hidden.type = "hidden", hidden.name = name, hidden.value = value, form.appendChild(hidden);
            return form.style.display = "none", document.body.appendChild(form), form
        },
        formData: function(args, type) {
            var data = {};
            for (var param in args) args[param] && ("password" === param ? data.password = utils.trim(args.password) : data[param] = utils.trim(args[param]));
            return data
        },
        getJSONP: function(url, callback, charset) {
            var ret, cbName = this.randomName("cb"),
                ...
                rts = /([?&])_=[^&]*/,
                ts = (new Date).getTime();
            url = url.replace(jsonp, "$1" + cbName), ret = url.replace(rts, "$1_=" + ts), url = ret + (ret === url ? (rquery.test(url) ? "&" : "?") + "_=" + ts : ""), window[cbName] = function(data) {
                utils.isFunction(callback) && callback(data), window[cbName] = void 0, head.removeChild(script)
            }, script.type = "text/javascript", script.charset = charset || "UTF-8", script.src = url, script.onerror = function(err) {
                var data = {
                    status: 500
                };
                window[cbName](data)
        ...
        formToIFrame: function(args, url) {
            ...
            if (args.callback) {
                ...
                window[cbName] = function(data) {
                    utils.isFunction(callback) && callback(data), window[cbName] = void 0, document.body.removeChild(that._form), document.body.removeChild(that._iframe)
                }
            }
            var formData = this.formData(args);
            formData.callback = cbName, isSohu && (document.domain = "sohu.com"), this._iframe = this.createIFrame(), this._form = this.createForm(formData), this._form.action = url, this._form.method = "POST", this._form.target = this._iframe.name, this._form.submit(), setTimeout(function() {
                if (window[cbName]) {
                    var data = {
                        status: 500
                    };
        ...
        bridging: function(args) {
            var option = {
                ...
                params = args.params || {};
            return params.appid = this.options.appid, params.callback = function(resp) {
                if (!resp) return void(option.error && option.error(resp));
                var data = resp.body;
        ...
        setCommonCookie: function() {
            if (!getCookie("gidinf") || !getCookie("reqtype")) {
                ...
                isSohu || (opt.domain = document.domain), url = utils.url(url, opt), this.getJSONP(url, function() {})
            }
        },
        getChallenge: function(back, args) {
            var callback = function(data) {
                    window.eval(data), args.params = args.params || {}, !isSohu && document._jv && (args.params._jv = document._jv), back && back(args)
                },
                opt = {
                    callback: "?",
                    type: isSohu ? 0 : 1
        ...
        login: function(args) {
            ...
            args.params.password = utils.md5(args.params.password), args.params.captcha && (args.params.pagetoken = pageTokenLogin), self.getChallenge(function(args2) {
        ...
        getMethod: function(args, url) {
            ...
            params.callback = "?", url = utils.url(url, params), this.getJSONP(url, callback)
        },
        getSignInCaptcha: function(args) {
            args.params.pagetoken = pageTokenPhone, this.getMethod(args, this.options.url.signInCaptcha)
        },
        getRegisterCaptcha: function(args) {
            args.params.pagetoken = pageTokenRegister, this.getMethod(args, this.options.url.registerCaptcha)
        },
        ...
        passportLogin: function(args) {
            ...
            args.params.password = utils.md5(args.params.password), self.getChallenge(function(args2) {
                self.formToIFrame(self.bridging(args2), url)
            }, args)
        },
    ...
    }, window.SohuPassport = SohuPassport, SohuPassport
});